...
import { SectorEnquiryForm, type SectorEnquiryConfig } from './SectorEnquiryForm';
import { ButtonLink } from '../ui/ButtonLink';
import { TextLink } from '../ui/TextLink';

export function SectorIntentPanel({ jobs, sector, shortName, employer }: { jobs: PublicJob[]; sector: string; shortName: string; employer: SectorEnquiryConfig }) {
  ...
  return <div className="relative isolate pt-14" data-sector-intent>
    <div role="tablist" aria-label="How can we help?" className="absolute right-3 top-3 z-0 grid h-14 w-[calc(100%-24px)] max-w-[440px] grid-cols-2 overflow-hidden rounded-t-[13px] border border-line bg-white max-[640px]:right-1 max-[640px]:w-[calc(100%-8px)]">
      ...
      {[employer.intent, 'I’m looking for work'].map((title, index) => <button key={title} ref={node => { tabs.current[index] = node; }} id={`sector-intent-${index}`} type="button" role="tab" aria-selected={intent === index} aria-controls={`sector-panel-${index}`} tabIndex={intent === index ? 0 : -1} onClick={() => setIntent(index)} onKeyDown={event => {
    ...
    <div className="relative z-10 grid">
      <div id="sector-panel-0" role="tabpanel" aria-labelledby="sector-intent-0" aria-hidden={intent !== 0} inert={intent !== 0} className={panelClass(0)}><SectorEnquiryForm config={employer} /></div>
      <div id="sector-panel-1" role="tabpanel" aria-labelledby="sector-intent-1" aria-hidden={intent !== 1} inert={intent !== 1} className={`${panelClass(1)} border border-line bg-surface`}>
        <div className="bg-ink p-8 text-white max-[640px]:p-5">
          <h2 className="text-[36px] font-semibold leading-none tracking-[-.045em]">Find {shortName.toLowerCase()} work</h2>
          <p className="mt-4 text-sm leading-[1.65] text-white/90">Explore current {sector} opportunities with Complex.</p>
        </div>
        <div className="p-8 max-[640px]:p-5">
          ...
          <div className="min-h-[230px]">
            ...
            {!matches.length && <p className="py-4 text-sm leading-relaxed text-muted">No matching roles. Try another location or work type, or register your interest below.</p>}
          </div>
          <ButtonLink href="/jobs" className="mt-5 w-full">View All {shortName} Jobs</ButtonLink>
