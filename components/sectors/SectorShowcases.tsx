...
export function HomeSectorGrid() {
  return <Container gutter="wide" className="pb-[120px] max-[760px]:pb-[88px]">
    <div className="grid gap-4 max-[760px]:grid-cols-1">
      {[sectors.slice(0, 2), sectors.slice(2, 4)].map((row, rowIndex) => <div className="homeSectorRow [container-type:inline-size] flex gap-4 max-[760px]:flex-col" key={rowIndex}>
        {row.map(sector => <a href={sector.href} aria-label={`Explore ${sector.title}`} className="homeSectorCard group/card relative isolate min-h-[500px] min-w-0 flex-1 overflow-hidden bg-ink text-white outline-none transition-[flex-basis] duration-700 ease-complex focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-brand-red max-[1000px]:min-h-[440px] max-[760px]:min-h-[390px]" key={sector.href}>
          ...
          <div className="absolute inset-x-7 bottom-8 min-w-0 max-[760px]:inset-x-6 max-[760px]:bottom-6">
            ...
            <div className="homeSectorCopy w-[calc(45cqw-64px)] max-[760px]:w-auto min-w-0 transition-transform duration-700 ease-complex group-hover/card:translate-x-[66px] group-focus-visible/card:translate-x-[66px] max-[1000px]:group-hover/card:translate-x-[56px] max-[1000px]:group-focus-visible/card:translate-x-[56px] max-[760px]:!translate-x-0">
              <div className="relative min-w-0">
                <h3 className="m-0 whitespace-nowrap text-[clamp(22px,3.3cqw,46px)] font-semibold leading-[.9] tracking-[-.055em] text-white max-[760px]:whitespace-normal max-[760px]:text-[clamp(34px,9vw,46px)]">{sector.displayTitle}</h3>
                <ArrowIcon className="absolute right-0 top-0 text-2xl text-white transition-transform duration-300 group-hover/card:translate-x-2 group-hover/card:-translate-y-2 group-focus-visible/card:translate-x-2 group-focus-visible/card:-translate-y-2" />
              </div>
              <p className="mb-0 mt-4 w-[min(470px,calc(45cqw-64px))] max-[760px]:w-auto text-xs font-normal leading-[1.6] text-white/80 max-[1000px]:max-w-[350px]">{sector.homeCopy}</p>
            </div>
          </div>
        </a>)}
      </div>)}
