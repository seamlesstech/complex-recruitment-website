...
export function ServiceHero({ kind, eyebrow, title, intro, actionHref, actionLabel, badgeLabel, badgeValue, badgeCopy, railItems }: ServiceHeroProps) {
  return <section className="relative isolate grid h-svh min-h-[760px] grid-rows-[92px_minmax(0,1fr)_96px] overflow-hidden bg-ink text-white max-[1000px]:h-auto max-[1000px]:min-h-0 max-[640px]:grid-rows-[76px_auto_auto]">
    <Header active="Compliance & Vetting" theme="dark" />
    ...
    <Container className="grid grid-cols-[1fr_240px] items-end gap-[70px] pb-[54px] pt-[72px] max-[1000px]:grid-cols-1 max-[1000px]:gap-[46px] max-[1000px]:py-[70px] max-[1000px]:pt-[90px] max-[640px]:py-[52px] max-[640px]:pb-11">
      <div className="max-w-[900px]"><div className="mb-[30px] text-[10px] font-extrabold tracking-[.2em] text-white before:mr-[13px] before:mb-[3px] before:inline-block before:h-0.5 before:w-9 before:bg-brand-red"><span className="text-white/60">COMPLIANCE &amp; VETTING / </span>{eyebrow}</div><h1 className="m-0 max-w-[900px] text-[clamp(62px,6vw,102px)] font-semibold leading-[.88] tracking-[-.065em] max-[640px]:text-[50px] [&_em]:not-italic [&_em]:text-brand-red">{title}</h1><p className="my-[30px] max-w-[600px] text-[17px] leading-[1.6] text-[#d5d7d8] max-[640px]:text-[15px]">{intro}</p><div className="flex items-center gap-8 max-[430px]:flex-col max-[430px]:items-start max-[430px]:gap-4"><ButtonLink href={actionHref}>{actionLabel}</ButtonLink><TextLink href="/compliance" variant="light" arrowDirection="right">Compliance Overview</TextLink></div></div>
